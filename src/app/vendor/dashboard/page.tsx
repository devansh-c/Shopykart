...
export default function VendorDashboard() {
  ...
  const orders = useMemo(() => {
    ...
    return rawOrders.filter((o: any) => {
      ...
      const isMainVendor = o.vendorId === vendorId;
      const isInVendorIds = Array.isArray(o.vendorIds) && o.vendorIds.includes(vendorId);
      const hasMyItems = o.items?.some((item: any) => String(item.vendorId) === vendorId);
    ...
    return (
      <div id={`receipt-content-${orderData.id}`} className="bg-white text-black p-5 font-mono text-[10px] uppercase leading-tight w-[280px] mx-auto border border-gray-100">
        ...
        <table className="w-full text-[9px]">
          <thead><tr className="border-b border-dashed border-black"><th className="text-left py-1" width="60%">ITEM</th><th className="text-center py-1" width="15%">QTY</th><th className="text-right py-1" width="25%">PRICE</th></tr></thead>
          <tbody>{orderData.items?.map((item: any, i: number) => (<tr key={i}><td className="py-2 pr-1 font-black leading-tight">{item.name}</td><td className="text-center">X{item.quantity}</td><td className="text-right">{(item.price * item.quantity).toFixed(2)}</td></tr>))}</tbody>
        </table>
  ...
  return (
    <div className="h-screen bg-[#F9FAFB] flex flex-col max-lg mx-auto shadow-2xl relative overflow-hidden">
      <header className="bg-white px-4 py-4 flex flex-col gap-3 border-b shrink-0 z-50">
         <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
               <span className={cn("text-[9px] font-black uppercase tracking-widest", vendorProfile?.isOnline !== false ? "text-green-600" : "text-red-500")}>{vendorProfile?.isOnline !== false ? 'OPEN' : 'OFFLINE'}</span>
              ...
              <div className="p-4 space-y-4 animate-in fade-in duration-500">
                  ...
                  {ordersLoading ? (
                    ...
                    filteredOrders.map(o => {
                      const myItems = o.items?.filter((item: any) => String(item.vendorId) === String(user?.uid)) || [];
                      return (
                        <div key={o.id} className="bg-white p-6 rounded-[2.5rem] border border-border/50 shadow-sm relative overflow-hidden group mb-4">
                          ...
                          <div className="bg-muted/30 rounded-2xl p-4 mb-4 space-y-3">
                              ...
                              <div className="space-y-1.5">{myItems.map((item:any, i:number) => (<div key={i} className="flex justify-between items-center text-[11px] font-bold"><span className="text-gray-700">{item.quantity}x {item.name}</span><span className="text-primary font-black">₹{(item.price * item.quantity).toFixed(2)}</span></div>))}</div>
                          </div>
                          <Dialog><DialogTrigger asChild><button className="w-full bg-white border-2 border-primary/20 text-primary h-11 rounded-xl font-black text-[9px] uppercase active:scale-95 transition-all flex items-center justify-center gap-1.5"><Eye className="h-3.5 w-3.5" /> View Bill</button></DialogTrigger>
                          <DialogContent className="rounded-[2.5rem] max-w-[340px] p-0 overflow-hidden bg-white flex flex-col max-h-[85vh] z-[11000]"><DialogHeader className="sr-only"><DialogTitle>Order Bill</DialogTitle></DialogHeader><div className="flex-1 overflow-y-auto no-scrollbar p-4 flex flex-col items-center"><div className="w-full scale-[1.05] origin-top mb-4">{generateReceiptDOM(o)}</div></div><div className="p-4 bg-gray-50 border-t flex gap-2 shrink-0"><Button className="flex-1 bg-black h-12 rounded-xl text-white font-black text-[10px] uppercase shadow-lg"><Printer className="h-4 w-4 mr-2" /> PRINT</Button><Button onClick={() => handleDownload(o)} disabled={downloadingId === o.id} className="flex-1 bg-primary h-12 rounded-xl text-white font-black text-[10px] uppercase shadow-lg">{downloadingId === o.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4 mr-2" />} SAVE</Button></div></DialogContent></Dialog>
                        </div>
                      )
                    })
                  ) : (
                    <div className="flex flex-col items-center justify-center py-20 text-center opacity-30">
                      ...
                      <p className="font-black italic uppercase tracking-widest text-xs uppercase">No orders in this category</p>
                    </div>
                  )}
              </div>
            )}
            ...
            {activeMainTab === 'account' && (
              <div className="p-4 space-y-6 animate-in fade-in duration-500">
                  <div className="flex flex-col items-center py-8">
                    ...
                    <h2 className="text-2xl font-black italic uppercase mt-6">{vendorProfile?.storeName}</h2>
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{vendorProfile?.category} Store</p>
                  </div>
      ...
      <Dialog open={isProductModalOpen} onOpenChange={setIsProductModalOpen}>
        <DialogContent className="rounded-[2.5rem] max-w-sm max-h-[85vh] overflow-y-auto no-scrollbar focus:outline-none border-none p-0">
           <DialogHeader className="p-6 pb-2 border-b"><DialogTitle className="font-black italic uppercase text-center text-xl">Manage Product</DialogTitle></DialogHeader>
           <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <Textarea placeholder="Short details" value={productForm.description} onChange={e => setProductProductForm({...productForm, description: e.target.value})} className="rounded-xl bg-muted/30 border-none h-24 p-4 text-xs" />
              </div>
