  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col max-lg mx-auto shadow-2xl relative">
      ...
      <Dialog open={showOrderAlarm} onOpenChange={setShowOrderAlarm}>
        <DialogContent className="rounded-[3rem] max-w-sm bg-[#0B0B0B] text-center border-rose-400/40 p-10 shadow-[0_0_80px_rgba(244,63,94,0.4)]">
          <DialogHeader className="sr-only">
            <DialogTitle>URGENT: NEW BEAUTY ORDER</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col items-center gap-8">
            <div className="relative">
              <div className="absolute inset-0 bg-rose-500/30 rounded-full animate-ping" />
              <div className="relative bg-rose-600 h-24 w-24 rounded-full flex items-center justify-center shadow-2xl">
                 <BellRing className="h-12 w-12 text-white animate-bounce" />
              </div>
            </div>
            <div className="space-y-2">
               <h2 className="text-4xl font-black italic uppercase text-white leading-none tracking-tighter">NEW<br />BOOKING!</h2>
            ...
            <div className="w-full bg-white/5 p-4 rounded-2xl border border-white/10 text-left">
               <div className="flex justify-between items-center mb-1">
                  <span className="text-[10px] font-black text-rose-400 uppercase">Order Details</span>
           ...
           <div className="p-4 space-y-4 animate-in fade-in duration-500">
              <div className="flex bg-white rounded-2xl p-1 shadow-sm mb-4 border border-border/50">
                {['NEW ORDERS', 'DELIVERED', 'CANCELLED'].map(f => (
                  <button key={f} onClick={() => setOrderFilter(f as OrderFilter)} className={cn("flex-1 py-3 text-[9px] font-black rounded-xl transition-all", orderFilter === f ? "bg-rose-600 text-white shadow-lg shadow-rose-100" : "text-gray-400")}>{f}</button>
                ))}
              </div>
              {orders?.filter(o => {
                ...
                <div key={o.id} className="bg-white p-5 rounded-[2rem] border border-border/50 shadow-sm relative overflow-hidden group">
                   ...
                   {orderFilter === 'NEW ORDERS' && (
                     <div className="flex gap-2">
                        {o.status === 'Placed' && (
                          <Button onClick={() => updateOrderStatus(o.id, 'Accepted')} className="flex-1 bg-black h-12 rounded-2xl font-black uppercase text-xs shadow-xl">Accept Booking</Button>
                        )}
                        {o.status === 'Accepted' && (
                          <Button onClick={() => updateOrderStatus(o.id, 'Preparing')} className="flex-1 bg-rose-600 h-12 rounded-2xl font-black uppercase text-xs text-white">Preparing Items</Button>
                        )}
                        {o.status === 'Preparing' && (
              ...
              {(!orders || orders.length === 0) && <div className="text-center py-20 opacity-20 flex flex-col items-center"><ShoppingBag className="h-16 w-16 mb-4" /><p className="font-black italic uppercase tracking-widest text-xs">No active beauty orders</p></div>}
           </div>
         ) : activeMainTab === 'catalog' ? (
           <div className="p-4 space-y-4 animate-in fade-in duration-500">
              <div className="flex justify-between items-center mb-4">
                 <h2 className="text-xl font-black italic uppercase">Beauty Catalog</h2>
                 <Dialog open={isAddOpen} onOpenChange={(val) => { setIsAddOpen(val); if(!val) resetForm(); }}>
                    <DialogTrigger asChild><Button className="bg-rose-600 text-white rounded-xl h-10 font-black uppercase text-[9px] tracking-widest"><Plus className="mr-1 h-3.5 w-3.5" /> ADD ITEM</Button></DialogTrigger>
                       ...
                       <div className="space-y-4 pt-4">
                          ...
                          <div className="space-y-3 pt-2">
                             <div className="flex items-center justify-between px-1">
                                <label className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2"><ListPlus className="h-3 w-3" /> Variants (e.g. 100ml, Shade 01)</label>
                                <Button type="button" onClick={handleAddOption} variant="ghost" className="h-7 text-[8px] font-black uppercase border border-rose-200 text-rose-600 rounded-lg">+ ADD VARIANT</Button>
                             </div>
                          ...
                          <Button onClick={handleAddProduct} disabled={isSubmitting} className="w-full h-16 bg-rose-600 text-white rounded-[1.5rem] font-black uppercase italic shadow-xl shadow-rose-100 text-lg">{isSubmitting ? 'Syncing...' : 'Publish to Hub'}</Button>
                       </div>
                    </DialogContent>
                 </Dialog>
              </div>
      ...
      <nav className="fixed bottom-0 max-w-lg mx-auto w-full bg-[#0F172A] pt-4 pb-8 px-6 flex justify-around border-t border-white/5 z-50 rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
        {[
          ...
          <button key={item.id} onClick={() => setActiveMainTab(item.id as MainTab)} className="flex flex-col items-center gap-1.5 active:scale-90 transition-none group">
            <item.icon className={cn("h-5 w-5 transition-none", activeMainTab === item.id ? "text-rose-400 scale-110" : "text-gray-500")} />
            ...
            {activeMainTab === item.id && <div className="h-1 w-1 bg-rose-400 rounded-full mt-0.5" />}
          </button>
        ))}
      </nav>
    </div>
