...
export default function BeautyDashboard() {
  ...
  const handleToggleStore = async (online: boolean) => {
    ...
    try {
      ...
      await batch.commit();
      toast({ title: online ? "Beauty Hub is Open! 🟢" : "Beauty Hub is Closed 🔴" });
    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col max-lg mx-auto shadow-2xl relative transform-gpu">
               ...
               filteredOrders?.map(o => (
                <div key={o.id} className="bg-white p-5 rounded-[2rem] border border-border/50 shadow-sm relative overflow-hidden group">
                   ...
                   <div className="bg-muted/30 rounded-2xl p-4 mb-4 space-y-2">
                      ...
                      {o.items?.filter((it:any) => it.vendorId === user?.uid).map((item:any, i:number) => (<div key={i} className="flex justify-between items-center text-xs font-bold"><span className="text-gray-700">{item.quantity}x {item.name}</span><span className="text-rose-600">₹{(item.price * item.quantity).toFixed(2)}</span></div>))}
                   </div>
                   <Dialog>
                    <DialogTrigger asChild><button className="w-full bg-white border-2 border-rose-200 text-rose-600 h-11 rounded-xl font-black text-[9px] uppercase active:scale-95 transition-all flex items-center justify-center gap-1.5"><Eye className="h-3.5 w-3.5" /> View Bill</button></DialogTrigger>
                    <DialogContent className="rounded-[2.5rem] max-w-[340px] p-0 overflow-hidden bg-white flex flex-col max-h-[85vh] z-[11000]"><DialogHeader className="sr-only"><DialogTitle>Order Bill</DialogTitle></DialogHeader><div className="flex-1 overflow-y-auto no-scrollbar p-4 flex flex-col items-center"><div className="scale-[1.05] origin-top mb-4">{generateReceiptDOM(o)}</div></div><div className="p-4 bg-rose-50 border-t flex gap-2 shrink-0"><Button onClick={() => handlePrint(o.id)} className="flex-1 bg-black h-12 rounded-xl text-white font-black text-[10px] uppercase shadow-lg"><Printer className="h-4 w-4 mr-2" /> PRINT</Button><Button onClick={() => handleDownload(o)} disabled={downloadingId === o.id} className="flex-1 bg-rose-600 h-12 rounded-xl text-white font-black text-[10px] uppercase shadow-lg">{downloadingId === o.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4 mr-2" />} SAVE</Button></div></DialogContent>
                   </Dialog>
                </div>
              ))}
           </div>
         ...
         {activeMainTab === 'catalog' && (
           <div className="p-4 space-y-4 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 gap-4">
                 {productsLoading ? <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-rose-600" /></div> : 
                  myProducts?.map(p => (
           ...
           <div className="p-4 space-y-6 animate-in fade-in duration-500">
              <div className="bg-[#0B0B0B] p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
                 <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-1"><Wallet className="h-4 w-4 text-rose-500" /><span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Beauty Wallet</span></div>
              ...
              <div className="space-y-4">
                 ...
                 {payoutsLoading ? <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-rose-600" /></div> :
                  payouts?.map(p => (
         ...
         {activeMainTab === 'account' && (
           <div className="p-4 space-y-6 animate-in fade-in duration-500">
              <div className="flex flex-col items-center py-8">
                 <div className="relative group"><div className="h-32 w-32 rounded-[2.5rem] border-4 border-white shadow-2xl overflow-hidden bg-muted">{vendorProfile?.imageUrl ? <img src={vendorProfile.imageUrl} className="h-full w-full object-cover" alt="" /> : <Sparkles className="h-12 w-12 m-auto text-gray-300" />}</div><button className="absolute bottom-[-10px] right-[-10px] bg-white p-3 rounded-2xl shadow-xl border border-border text-rose-600"><Camera className="h-5 w-5" /></button></div>
                 <h2 className="text-2xl font-black italic uppercase tracking-tighter mt-6">{vendorProfile?.storeName}</h2>
                 <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Cosmetics Provider • {vendorProfile?.town}</p>
              </div>
              <div className="bg-white p-6 rounded-[2.5rem] border border-border/50 shadow-sm space-y-5">
                 <div className="space-y-1"><label className="text-[9px] font-black uppercase text-gray-400 ml-1">Store Name</label><Input value={profileForm.storeName} onChange={e => setProfileForm({...profileForm, storeName: e.target.value})} className="h-12 rounded-xl bg-gray-50 border-none font-bold" /></div>
      ...
      <Dialog open={isProductModalOpen} onOpenChange={setIsProductModalOpen}>
        <DialogContent className="rounded-[2.5rem] max-w-sm max-h-[85vh] overflow-y-auto no-scrollbar focus:outline-none border-none p-0">
           <DialogHeader className="p-6 pb-2 border-b"><DialogTitle className="font-black italic uppercase text-center text-xl">Beauty Manager</DialogTitle></DialogHeader>
           <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                   ...
                   <Input placeholder="Price ₹" type="number" value={productForm.price} onChange={e => setProductProductForm({...productForm, price: e.target.value})} className="h-12 rounded-xl border-rose-400/40" />
                </div>
                ...
                <Textarea placeholder="Product Details" value={productForm.description} onChange={e => setProductProductForm({...productForm, description: e.target.value})} className="rounded-xl bg-muted/30 border-none h-24 p-4 text-xs" />
              </div>
              <Button onClick={handleSaveProduct} disabled={isSavingProduct} className="w-full h-16 bg-rose-600 rounded-3xl font-black uppercase italic shadow-xl text-white">{isSavingProduct ? <Loader2 className="h-6 w-6 animate-spin" /> : editingProduct ? 'UPDATE PRODUCT' : 'PUBLISH PRODUCT'}</Button>
           </div>
        </DialogContent>
      </Dialog>
    </div>
