...
import { Bike, Mail, Lock, Loader2, ArrowRight, Apple, ChevronLeft } from 'lucide-react';
import { useAuth, useFirestore } from '@/firebase';
import { signInWithEmailAndPassword, signOut, GoogleAuthProvider, OAuthProvider, signInWithPopup, sendPasswordResetEmail } from 'firebase/auth';
...
export default function DeliveryLoginPage() {
  ...
  const handleSocialAuth = async (providerName: 'google' | 'apple') => {
    ...
    try {
      ...
      const partnerRef = doc(firestore, 'delivery_partners', firebaseUser.uid);
      ...
      if (!partnerSnap.exists()) {
        ...
        toast({ 
          variant: "destructive", 
          title: "Access Denied", 
          description: "No partner profile found for this social account." 
        });
      } else {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        ...
        toast({ variant: "destructive", title: "Login Failed", description: "Use email and password login." });
      }
    } finally {
            ...
            {view === 'login' && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                   <button type="button" onClick={() => handleSocialAuth('google')} disabled={!!socialLoading} className="h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center gap-2 hover:bg-white/10 active:scale-95 transition-all">
                   ...
                   <button type="button" onClick={() => handleSocialAuth('apple')} disabled={!!socialLoading} className="h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center gap-2 hover:bg-white/10 active:scale-95 transition-all">
                      {socialLoading === 'apple' ? <Loader2 className="h-4 w-4 animate-spin text-white" /> : (
                        <>
                          <Apple className="h-4 w-4 fill-white text-white" />
                          <span className="text-[8px] font-black uppercase text-white">Apple</span>
                        </>
                      )}
                   </button>
                </div>
