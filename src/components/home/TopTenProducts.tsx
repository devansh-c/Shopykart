    ...
    <div className="py-6 overflow-hidden">
      ...
      <div className="flex overflow-x-auto space-x-12 px-8 no-scrollbar pb-8 pt-4">
        {filteredTopProducts.map((product, index) => {
          ...
          return (
            <div key={product.id} className={cn("relative min-w-[150px] group transition-all", isOffline && "grayscale opacity-80")}>
              <div className="absolute -left-10 bottom-0 text-[160px] font-black italic opacity-20 pointer-events-none" style={{ WebkitTextStroke: '2px #333', color: 'transparent', zIndex: 0 }}>{index + 1}</div>
              <div className="relative z-10 bg-white rounded-2xl overflow-hidden shadow-xl border border-border/40 active:scale-95">
                <div onClick={() => !isOffline && router.push(`/product/view?id=${product.id}`)} className="cursor-pointer">
                  <div className="relative aspect-[3/4] w-full bg-muted">
                    <Image src={imageUrl} alt={product.name} fill className="object-cover" unoptimized />
                    {isOffline && (
                      <div className="absolute inset-0 bg-black/60 z-30 flex items-center justify-center p-2"><span className="text-white font-black text-[10px] uppercase border-2 border-white/30 px-2 py-1 rounded-lg">Closed</span></div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <div className="absolute bottom-0 left-0 right-0 p-3"><h3 className="text-white text-xs font-black uppercase italic leading-none truncate">{product.name}</h3></div>
                  </div>
                </div>
                {!isOffline && (
                  <button onClick={() => addToCart({ ...product, imageUrl })} className="absolute top-2 right-2 bg-white/90 backdrop-blur-md p-1.5 rounded-lg text-primary shadow-lg"><Plus className="h-4 w-4" /></button>
                )}
              </div>
            </div>
          );
