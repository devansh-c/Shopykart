        ...
        <div className="mt-auto flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1">
               ...
               <span className="text-[7px] font-bold text-gray-400">{(Number(product.rating) || 4.5).toFixed(1)}</span>
            </div>
          </div>
          {!isOffline ? (
            quantity === 0 ? (
...
export function PopularProducts({ 
  ...
  const productsToDisplay = useMemo(() => {
    ...
    const filtered = list.filter(p => {
      const vendorMatch = vendorMap.get(p.vendorId);
  ...
  return (
    <div className="px-4 py-6 transform-gpu min-h-[400px]">
      ...
      {productsLoading && productsToDisplay.length === 0 && (
        <div className="grid grid-cols-2 gap-4">
           {[1, 2, 3, 4].map(i => <div key={i} className="bg-gray-100 animate-pulse h-60 rounded-[2.5rem]" />)}
        </div>
      )}
    </div>
  );
