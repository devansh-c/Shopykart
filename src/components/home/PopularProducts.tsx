    ...
    return (
      <div className={cardClasses}>
        ...
        <ProductQuickView product={product} isMedical={activeMode === 'Medical'}>
          <div className="relative aspect-square w-full rounded-2xl overflow-hidden mb-3 bg-muted/50 cursor-pointer">
        ...
        <ProductQuickView product={product} isMedical={activeMode === 'Medical'}>
          <div className="text-left flex flex-col gap-0.5 mb-3 h-14 cursor-pointer">
        ...
        <div className="mt-auto flex items-center justify-between">
           ...
           <div className="relative">
              {quantity === 0 ? (
                <button disabled={isOffline} onClick={(e) => { e.stopPropagation(); if(!isOffline) onAdd({ ...product, imageUrl }); }} className={cn("px-6 py-1.5 border rounded-lg font-black text-[10px] uppercase shadow-sm transition-all active:scale-95", isOffline ? "border-gray-200 text-gray-300" : "border-primary text-primary hover:bg-primary/5")}>
                  {isOffline ? 'OFF' : 'ADD'}
                </button>
              ) : (
                <div className={cn("flex items-center bg-primary text-white rounded-lg h-8 px-1 shadow-md", isOffline && "opacity-50")}>
                  <button onClick={(e) => { e.stopPropagation(); onRemove(product.id); }} className="w-6 h-full flex items-center justify-center font-bold text-lg">-</button>
                  <span className="w-5 text-center text-[11px] font-black">{quantity}</span>
                  <button disabled={isOffline} onClick={(e) => { e.stopPropagation(); if(!isOffline) onAdd({ ...product, imageUrl }); }} className="w-6 h-full flex items-center justify-center font-bold text-lg">
    ...
    <div className={cardClasses}>
      <div className="flex-1 pr-4 min-w-0">
        ...
        <ProductQuickView product={product} isMedical={false}>
          <div className={cn("block text-left w-full cursor-pointer", isOffline && "pointer-events-none")}>
      ...
      <div className="relative w-28 h-28 shrink-0">
        <ProductQuickView product={product} isMedical={false}>
          <div className="relative w-full h-full rounded-2xl overflow-hidden bg-muted shadow-inner cursor-pointer">
        ...
        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-full px-1.5 z-20">
          {quantity === 0 ? (
            <ProductQuickView product={product} isMedical={false}>
              <button disabled={isOffline} onClick={(e) => { e.stopPropagation(); if(!isOffline) onAdd({ ...product, imageUrl }); }} className={cn("w-full h-9 bg-white shadow-lg font-black text-[9px] uppercase rounded-xl transition-all active:scale-95", isOffline ? "text-gray-300 border-2 border-gray-200" : "text-primary border-2 border-primary")}>
                {isOffline ? 'OFF' : 'ADD'}
              </button>
            </ProductQuickView>
          ) : (
            <div className={cn("flex items-center justify-between w-full h-9 bg-primary text-white rounded-xl shadow-lg", isOffline && "opacity-50")}>
              <button onClick={(e) => { e.stopPropagation(); onRemove(product.id); }} className="flex-1 flex items-center justify-center h-full"><Minus className="h-3 w-3" /></button>
              <span className="text-xs font-black min-w-[20px] text-center">{quantity}</span>
              <button disabled={isOffline} onClick={(e) => { e.stopPropagation(); if(!isOffline) onAdd({ ...product, imageUrl }); }} className="flex-1 flex items-center justify-center h-full">
