...
const ProductItem = memo(({ product, quantity, isOffline, onShare, onAdd, onRemove, isGuest }: any) => {
  const displayPrice = isGuest ? Math.max(0, (product.price || 0) - 10) : (product.price || 0);
    ...
    <div className={cn("relative bg-[#0B0B0B] rounded-[2.5rem] p-3 border-2 border-[#C5A021]/30 flex flex-col shadow-2xl transition-all transform-gpu", isOffline && "opacity-75 grayscale-[0.5]")}>
      <div className="relative aspect-square w-full mb-3">
        <ProductQuickView product={product} vendorScheduleOpen={!isOffline}>
           <div className="relative w-full h-full cursor-pointer overflow-hidden rounded-[1.5rem] border-2 border-white/5">
              ...
              {isGuest && !isOffline && <div className="absolute top-2 left-2 bg-primary text-white text-[8px] font-black px-2 py-0.5 rounded-full animate-pulse shadow-lg">₹10 OFF</div>}
           </div>
        </ProductQuickView>
      ...
      <div className="flex-1 flex flex-col px-1">
        ...
        <div className="mt-auto flex items-center justify-between pt-2">
          <div className="flex flex-col"><span className="text-lg font-black text-white italic tracking-tighter leading-none">₹{displayPrice}</span><div className="flex items-center gap-1 mt-1"><Star className="h-2.5 w-2.5 fill-amber-400 text-amber-400" /><span className="text-[8px] font-bold text-gray-400">{(Number(product.rating) || 4.5).toFixed(1)}</span></div></div>
          {!isOffline ? (quantity === 0 ? (<ProductQuickView product={product} vendorScheduleOpen={true}><button className="bg-[#D9C4A9] text-[#451A03] h-9 px-6 rounded-full font-black text-[10px] uppercase shadow-lg active:scale-90">ADD</button></ProductQuickView>) : (<div className="flex items-center bg-[#C5A021] text-[#451A03] rounded-full h-9 px-1.5 shadow-xl border border-white/20"><button onClick={() => onRemove(product.id)} className="w-7 h-full flex items-center justify-center"><Minus className="h-4 w-4 stroke-[3]" /></button><span className="text-[11px] font-black w-5 text-center">{quantity}</span><button onClick={() => onAdd({...product, price: displayPrice, quantity: 1})} className="w-7 h-full flex items-center justify-center"><Plus className="h-4 w-4 stroke-[3]" /></button></div>)) : (<div className="bg-gray-800 text-gray-500 h-9 px-4 rounded-full font-black text-[8px] uppercase flex items-center border border-white/5">OFFLINE</div>)}
        </div>
      </div>
    </div>
  );
...
export function PopularProducts({ searchQuery = '', category = 'all', activeMode = 'Food', initialData = [], initialStores = [] }: { searchQuery?: string, category?: string, activeMode?: string, initialData?: any[], initialStores?: any[] }) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => { if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 1000) setVisibleCount(p => p + 40); };
  ...
  return (
    <div className="px-4 py-6 bg-white min-h-[600px]">
      <div className="flex items-center justify-between mb-6 px-2"><h2 className="text-2xl font-black italic uppercase tracking-tighter text-gray-900">All <span className="text-primary">Products</span></h2></div>
