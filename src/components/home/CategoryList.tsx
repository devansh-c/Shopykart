...
/**
 * @fileOverview CategoryList - Optimized for Instant Paint.
 * Removed all mounting gates to ensure categories appear with banners.
 */
export function CategoryList({ 
  ...
  // If no data and loading, show skeleton immediately
  if (!dbCategories && loading) {
    return (
        ...
        <div className="flex space-x-4 no-scrollbar px-2">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="flex flex-col items-center gap-2 shrink-0">
              ...
              <div className="w-12 h-2 bg-muted/10 animate-pulse rounded-full" />
            </div>
          ))}
        </div>
      </div>
