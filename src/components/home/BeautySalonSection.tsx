...
/**
 * @fileOverview Beauty & Cosmetics compact card for grid layout.
 */
export function BeautySalonSection({ onClick }: { onClick?: () => void }) {
  return (
    <button 
      onClick={onClick}
      className="relative block w-full group overflow-hidden rounded-[2rem] h-44 shadow-2xl shadow-rose-200/50 active:scale-[0.98] transition-all duration-300 border border-white/10 text-left"
    >
      {/* Main Gradient Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#F43F5E] via-[#FB7185] to-[#EC4899] group-hover:scale-105 transition-transform duration-700" />
      ...
      <div className="relative h-full z-10 p-5 flex flex-col justify-between">
        ...
        <div>
          <h3 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">
            Beauty & <span className="text-rose-100">Cosmetics</span>
          </h3>
