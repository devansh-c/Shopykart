...
export function StoreSection() {
  ...
  // Fetch all approved vendors and filter client-side to avoid index issues
  ...
  const filteredVendors = useMemo(() => {
    ...
    // Filter by approved status and town (if town is selected)
    let result = dbVendors.filter(v => v.status === 'approved');
    
    if (currentTown) {
      const townMatch = result.filter(v => v.town === currentTown);
      // If no match in town, show all approved ones so it doesn't look empty
