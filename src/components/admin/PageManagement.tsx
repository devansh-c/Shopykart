...
import { Plus, Trash2, Edit, FileText, Loader2, Info, Check, X, Globe, Link as LinkIcon } from 'lucide-react';
...
export default function PageManagement() {
  ...
  const handleSave = async () => {
    ...
    const manualSlug = formData.slug.trim().toLowerCase().replace(/\s+/g, '-');
    const finalSlug = manualSlug || slugify(formData.title);
    ...
    try {
      if (editingId) {
        ...
        toast({ title: "Page Published", description: "Visible on ShopyKart now." });
      }
      setIsAddOpen(false);
      resetForm();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (!firestore) return;
    if (confirm("Delete this page permanently? This will hurt SEO if the link is shared.")) {
  ...
  const copyLink = (page: any) => {
    ...
    const url = `${window.location.origin}/pages/${slug}-${page.id}`;
    navigator.clipboard.writeText(url);
    toast({ title: "SEO Link Copied!", description: "Ready to share or link." });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-black italic uppercase">Dynamic Content</h2>
          <p className="text-xs text-muted-foreground font-bold">SEO Friendly Policy & Info Pages</p>
        </div>
        <Dialog open={isAddOpen} onOpenChange={(val) => { setIsAddOpen(val); if(!val) resetForm(); }}>
          ...
          <DialogContent className="rounded-[2.5rem] max-w-2xl h-[80vh] flex flex-col p-0 overflow-hidden">
            <DialogHeader className="p-6 pb-2">
              <DialogTitle className="font-black italic uppercase text-center">{editingId ? 'Edit SEO Page' : 'Page Composer'}</DialogTitle>
            </DialogHeader>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 pt-2 space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <Input value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="e.g. Refund Policy" className="h-12 rounded-xl font-bold" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-primary ml-1 flex items-center gap-1.5"><Globe className="h-2.5 w-2.5" /> SEO URL Slug</label>
                    <Input value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} placeholder="refund-policy" className="h-12 rounded-xl bg-primary/5 border-primary/10 font-black italic" />
                  </div>
               </div>

               <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Content Editor (HTML Support)</label>
                  <Textarea 
                    value={formData.content} 
                    onChange={e => setFormData({...formData, content: e.target.value})} 
                    placeholder="Write page content. You can use <b>, <br>, <ul>, etc." 
                    className="min-h-[350px] rounded-2xl font-medium p-6 bg-muted/10 border-none"
                  />
               </div>
               ...
               <div className="bg-blue-50 p-4 rounded-2xl flex gap-3 border border-blue-100">
                  ...
                  <p className="text-[9px] font-bold text-blue-800 uppercase leading-relaxed">
                    URLs automatically SEO optimized honge. Title se slug generate hoga agar aap manually nahi bharenge.
                  </p>
               </div>
            </div>
            ...
            <div className="p-6 bg-muted/5 border-t">
               <Button onClick={handleSave} disabled={isProcessing} className="w-full h-16 rounded-[2rem] bg-primary font-black uppercase italic shadow-xl text-lg">
                 {isProcessing ? <Loader2 className="h-6 w-6 animate-spin" /> : editingId ? 'UPDATE LIVE PAGE' : 'PUBLISH & INDEX PAGE'}
               </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading && !pages ? (
              ...
              <div className="flex items-center gap-4 min-w-0">
                 ...
                 <div className="min-w-0">
                    ...
                    <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest truncate">
                      /{page.slug || slugify(page.title)}-{page.id.slice(-4)}
                    </p>
                 </div>
              </div>
              <div className="flex gap-2 shrink-0">
                 <Button onClick={() => copyLink(page)} variant="ghost" size="icon" className="h-10 w-10 rounded-xl bg-primary/5 text-primary">
          ...
          <div className="col-span-full text-center py-20 bg-muted/20 rounded-[2.5rem] border-2 border-dashed">
            ...
            <p className="text-muted-foreground font-black italic uppercase tracking-widest text-sm">No SEO pages created yet</p>
          </div>
        )}
      </div>
    </div>
