...
/**
 * @fileOverview ULTIMATE EMERGENCY ALARM SYSTEM v8 - WITH AUTO PERMISSION REQUEST.
 * Optimized for lightning-fast detection and notification permission handling for APK.
 */
export default function NotificationHandler() {
  ...
  // 0. AUTO PERMISSION REQUEST ON MOUNT
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!audioRef.current || !isManagementPath || (userRole !== 'admin' && userRole !== 'vendor')) return;
    if (isRinging && !isAudioContextBlocked) {
      const playPromise = audioRef.current.play();
      if (playPromise !== undefined) playPromise.catch(() => setIsAudioContextBlocked(true));
      if (typeof window !== 'undefined' && window.navigator.vibrate) {
        const pattern = [400, 100, 400];
        window.navigator.vibrate(pattern);
        vibInterval = setInterval(() => window.navigator.vibrate(pattern), 1200);
      }
    } else {
      if (audioRef.current) { audioRef.current.pause(); audioRef.current.currentTime = 0; }
      if (typeof window !== 'undefined' && window.navigator.vibrate) window.navigator.vibrate(0);
    }
      ...
      const unsubCustomer = onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === 'modified') {
            ...
            if (prev && prev !== order.status && !pathname?.includes('/orders/track')) setCustomerUpdate(order);
          }
  ...
  const handleManualUnblock = (e: React.MouseEvent) => {
    ...
    if (audioRef.current) {
      ...
      if (playPromise !== undefined) {
        ...
        }).catch(() => toast({ variant: "destructive", title: "Tap again to enable sound" }));
      }
    }
  };
    ...
    <>
      {(userRole === 'admin' || userRole === 'vendor') && isManagementPath && isAudioContextBlocked && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[999999] w-full max-w-sm px-4">
           <button onClick={handleManualUnblock} className="w-full bg-black text-white border-2 border-primary rounded-[2.5rem] p-6 shadow-2xl flex items-center gap-5 hover:bg-primary active:scale-95 group">
              ...
              <div className="flex flex-col items-start text-left">
                ...
                <span className="text-[14px] font-black italic uppercase text-white mt-2">TAP TO ENABLE ALERTS</span>
              </div>
           </button>
        </div>
      )}
      ...
      <Dialog open={ringingOrders.length > 0} onOpenChange={() => {}}>
        <DialogContent className="rounded-[3.5rem] max-w-sm p-0 overflow-hidden border-none shadow-[0_0_120px_rgba(239,68,68,0.4)] bg-white z-[55000]">
          <DialogHeader className="sr-only"><DialogTitle>New Order</DialogTitle></DialogHeader>
          ...
          <div className="p-10 space-y-10 flex flex-col items-center text-center">
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 bg-red-600 text-white p-3 rounded-2xl shadow-2xl border-4 border-white animate-pulse"><AlertTriangle className="h-8 w-8" /></div>
            </div>
            <div className="space-y-4">
              <h2 className="text-5xl font-black italic uppercase tracking-tighter leading-none text-red-600">ALARM ON.<br /><span className="text-black">NEW ORDER!</span></h2>
            </div>
               ...
               {ringingOrders.slice(0, 1).map((order) => (
                 <div key={order.id} className="bg-gray-50 p-6 rounded-[2.5rem] border border-gray-200 text-left relative overflow-hidden group shadow-inner">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-[11px] font-black text-gray-400 uppercase tracking-widest">ID: #{order.customerOrderNumber || order.id.slice(-4)}</span>
                      <span className="text-3xl font-black italic text-red-600 tracking-tighter">₹{order.total?.toFixed(0)}</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="h-12 w-12 rounded-2xl bg-white border border-gray-200 flex items-center justify-center shadow-md"><User className="h-6 w-6 text-primary" /></div>
                      <div className="flex flex-col">
                         <span className="text-sm font-black text-gray-800 uppercase italic truncate max-w-[180px]">{order.customerName}</span>
                         <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{order.items?.length || 0} Items</span>
                      </div>
                    </div>
                 </div>
               ))}
            </div>
            <Button onClick={() => handleAcceptOrder(ringingOrders[0].id)} disabled={isAccepting} className="w-full h-24 bg-green-600 hover:bg-green-700 text-white rounded-[2.5rem] font-black uppercase italic text-4xl shadow-xl active:scale-95 transition-all border-b-[10px] border-green-800">
              {isAccepting ? <Loader2 className="h-12 w-12 animate-spin" /> : "ACCEPT"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      {userRole === 'customer' && customerUpdate && (
        <Dialog open={!!customerUpdate} onOpenChange={(val) => !val && setCustomerUpdate(null)}>
           <DialogContent className="rounded-[3rem] max-w-sm p-0 overflow-hidden border-none shadow-2xl bg-white z-[55000]">
              <DialogHeader className="sr-only"><DialogTitle>Update</DialogTitle></DialogHeader>
              <div className="p-10 space-y-8 text-center flex flex-col items-center">
                 <div className="relative">
                    <div className="absolute inset-0 bg-primary/10 blur-3xl rounded-full animate-pulse" />
                    <div className="relative h-24 w-24 bg-primary rounded-[1.75rem] flex items-center justify-center text-white shadow-2xl shadow-primary/20"><Package className="h-12 w-12 animate-bounce" /></div>
                    <div className="absolute -top-3 -right-3 bg-green-500 text-white p-2 rounded-xl shadow-lg border-4 border-white animate-in zoom-in duration-500"><Zap className="h-4 w-4" /></div>
                 </div>
                 <div className="space-y-2">
                    <h2 className="text-3xl font-black italic uppercase tracking-tighter text-gray-900 leading-none">ORDER UPDATE!</h2>
                    <p className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">Order #{customerUpdate?.customerOrderNumber} is now</p>
                 </div>
                 <div className="bg-primary/5 px-10 py-4 rounded-[1.5rem] border-2 border-dashed border-primary/20 inline-block">
                    <span className="text-2xl font-black italic uppercase text-primary tracking-tighter">{customerUpdate?.status}</span>
                 </div>
                 <Button onClick={() => { router.push(`/order/track/#${customerUpdate.customerOrderNumber}`); setCustomerUpdate(null); }} className="w-full h-18 bg-[#0B0B0B] hover:bg-primary text-white rounded-[2rem] font-black uppercase italic shadow-2xl text-xl">TRACK LIVE NOW</Button>
              </div>
           </DialogContent>
        </Dialog>
      )}
